...
def fetch_data():
    supabaseList = supabase.table('maintable2').select('*').execute().data
...
while True:
    ...
    st.plotly_chart(px.line(df_recent, x="DateTime", y="temperature", title='Smoke Sensor Readings', markers=True), use_container_width=True)

    st.plotly_chart(px.scatter(df_recent, x="DateTime", y="temperature", title='Smoke Sensor Readings'), use_container_width=True)

    st.plotly_chart(px.bar(df_recent, x="DateTime", y="temperature", title='Smoke Sensor Readings'), use_container_width=True)
    
    time.sleep(5)  # Wait for 5 seconds before fetching new data
    
    # Calculate maximum, minimum, and average values
    max_value = df_recent["temperature"].max()
    min_value = df_recent["temperature"].min()
    avg_value = df_recent["temperature"].mean()
